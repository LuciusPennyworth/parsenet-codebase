...
class PrimitivesEmbeddingDGCNGn(nn.Module):
    ...
    def __init__(self, opt, emb_size=50, num_primitives=8, primitives=True, embedding=True, parameters=True, mode=0, num_channels=3, nn_nb=80):
        ...
        # offset
        self.mlp_offset_prob1 = torch.nn.Conv1d(256, 256, 1)
        self.mlp_offset_prob2 = torch.nn.Conv1d(256, 3, 1)
        self.bn_offset_prob1 = nn.GroupNorm(4, 256)
    ...
    def positional_encoding(self, points, N_freqs):
        ...
        return torch.cat([fn(points) for fn in embed_fns], -1)
    ...
    def forward(self, points, normals, end_points=None, inds=None, postprocess=False):
        ...
        if postprocess:
            np.random.seed(1234)
        np.random.shuffle(l)
        random_index = torch.from_numpy(l[:self.opt.point_nums])

        input_val = torch.cat([points, normals], dim=-1)
        subidx = random_index.to(input_val.device).long().view(1,1,-1).repeat(batch_size,1,1)
        input_val = torch.gather(input_val.permute(0, 2, 1), -1, subidx.repeat(1,6,1)) # [b, 6, n_sub]

        num_points = input_val.shape[2]
        x, first_layer_features = self.encoder(input_val)
        ...
        # # offset
        x = F.dropout(F.relu(self.bn_offset_prob1(self.mlp_offset_prob1(x_all))), self.drop)
        offset_per_point = self.mlp_offset_prob2(x).permute(0, 2, 1)

        sub_points = torch.gather(points.permute(0, 2, 1), -1, subidx.repeat(1, 3, 1)).permute(0, 2, 1)
        after_vote = sub_points + offset_per_point
        after_vote = self.positional_encoding(after_vote, 1)
        # ms_feat = torch.cat([after_vote, embedding], dim=2)
        ms_feat = embedding

        return ms_feat, type_per_point, normal_per_point, param_per_point, offset_per_point, subidx.squeeze(1)
...
class PrimitiveNet(nn.Module):
    ...
    def forward(self, xyz, normal, inds=None, postprocess=False):
        ...
        feat_spec_embedding, T_pred, normal_per_point, T_param_pred, offset_per_point, subidx = self.affinitynet(
                xyz.transpose(1, 2).contiguous(),
                normal.transpose(1, 2).contiguous(),
                inds=inds,
                postprocess=postprocess)

        return feat_spec_embedding, T_pred, normal_per_point, T_param_pred, offset_per_point, subidx
