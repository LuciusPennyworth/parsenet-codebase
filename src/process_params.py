...
import h5py
import argparse
from src.primitive_forward import *
...
parser.add_argument('--data_path', type=str, default='/home/zhuhan/Code/ProjectMarch/dataset/parsenet/data/shapes')
parser.add_argument('--save_path', type=str, default='/home/zhuhan/Code/ProjectMarch/dataset/parsenet/data/with_params')
...
for i in range(len_):
    ...
    for j in range(n_instances):
        ...
        if primitive_type == 5:  # fitting sphere
            center, radius = fit.fit_sphere_torch(points, normals, weights)
            if radius > 10:
            ...
            a, d = fit.fit_plane_torch(points, normals, weights)
            ...
            try:
                a, center, radius = fit.fit_cylinder_torch(points, normals, weights)
            except:
            ...
            center, a, theta = fit.fit_cone_torch(points, normals, weights)
            if center[0][0] > 10 or center[1][0] > 10 or center[2][0] > 10:
                tmp = center.cpu().numpy()
                print(filename + ': cone center:' + str(tmp[0][0]) + ' ' + str(tmp[1][0]) + ' ' + str(
                    tmp[2][0]) + 'point number:' + str(N))
                continue
    ...
    wf.create_dataset('param', data=primitive_param)