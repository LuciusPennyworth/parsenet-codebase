...
class PrimitivesEmbeddingDGCNGn(nn.Module):
    ...
    def __init__(self, emb_size=128, num_primitives=10, primitives=True, embedding=True, mode=5, num_channels=6,
                 parameters=True, offset=True, loss_function=None, nn_nb=80):
    ...
    def forward(self, inputs, labels, compute_loss=True):  # inputs bs, 6, 8k
        batch_size = inputs.shape[0]
        num_points = inputs.shape[2]
        points = inputs[:,:3,:]
        x, first_layer_features = self.encoder(inputs)
        ...
        if self.mode == 5:
            ...
            normal_norm = torch.norm(normal_per_point, dim=-1, keepdim=True).repeat(1, 1, 3) + 1e-12
        ...
        after_vote = points.permute(0, 2, 1) + offset_per_point
        # after_vote = self.positional_encoding(after_vote, 1)
        ms_feat = torch.cat([embedding, after_vote.permute(0,2,1)], dim=1)

        embed_loss = self.loss_function(ms_feat, labels.data.cpu().numpy())

        return ms_feat, primitives_log_prob, embed_loss, normal_per_point, param_per_point, offset_per_point
